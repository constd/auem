...
class TestPredictionReport:
    ...
    def example_preds_df(self, request, n_examples, n_sources, n_classes):
        df = pd.DataFrame([generate_example(n_classes, n_sources=n_sources) for _ in range(n_examples)])
        assert len(df) == n_examples
        return df
    
    def test_prediction_report_df(self, example_preds_df, n_examples, n_sources):
        result_df = au_eval.prediction_report_df(example_preds_df,
    ...
    def test_prediction_report_df__with_formatting(self, example_preds_df, n_examples, n_sources):
        result_df = au_eval.prediction_report_df(example_preds_df,
                                                 with_formatting=True)
        assert isinstance(result_df, pd.io.formats.style.Styler)
        assert isinstance(result_df._repr_html_(), str)

    def test_prediction_report_df__with_source(self, example_preds_df, n_examples, n_sources, n_classes):
        result_df = au_eval.prediction_report_df(example_preds_df, source_column="source", with_formatting=False)
        assert isinstance(result_df, pd.DataFrame)
        assert len(result_df) == (len(example_preds_df["y_true"].unique()) + 1)
